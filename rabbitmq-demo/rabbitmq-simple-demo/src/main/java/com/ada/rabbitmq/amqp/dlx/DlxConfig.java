...
public class DlxConfig {
    ...
    public DirectExchange exchange() {
        ...
        return new DirectExchange("Ada_ORI_USE_EXCHANGE", true, false, new HashMap<>());
    }
    ...
    public Queue queue() {
        ...
        map.put("x-dead-letter-exchange", "Ada_DEAD_LETTER_EXCHANGE"); // 队列中的消息变成死信后，进入死信交换机
        return new Queue("Ada_ORI_USE_QUEUE", true, false, false, map);
    }

    @Bean
    public Binding binding(@Qualifier("oriUseQueue") Queue queue,@Qualifier("oriUseExchange") DirectExchange exchange) {
        return BindingBuilder.bind(queue).to(exchange).with("ada.ori.use");
    }
    ...
    public TopicExchange deadLetterExchange() {
        return new TopicExchange("Ada_DEAD_LETTER_EXCHANGE", true, false, new HashMap<>());
    }

    @Bean("deatLetterQueue")
    public Queue deadLetterQueue() {
        return new Queue("Ada_DEAD_LETTER_QUEUE", true, false, false, new HashMap<>());
    }
